...
if ($BuildOnly -or (-not $Install)) {
    ...
    Write-Host ""
    Write-Host "Signed outputs:"
    Write-Host "  Kernel:      $KernelOutput"
    Write-Host "  Minifilter:  $MiniOutput"
    Write-Host "  Controller:  $ControllerOutput"
    Write-Host "  Probe:       $ProbeOutput"
    Write-Host "  Certificate: $($certPaths.Cer)"
    Write-Host ""
    Write-Host "To deploy to a VM, copy these files and the .cer to the VM,"
